...
import { getCustomerByUserId, getDriverByUserId, listAvailableDrivers, updateDriverProfile } from '@/lib/services/accounts';
...
function deriveRouteType(pickupLocation: string, dropoffLocation: string): RouteType {
  ...
  return text.includes('da nang') || text.includes('da nẵng') || text.includes('đà nẵng')
    ? 'da_nang_to_dai_loc'
    : 'dai_loc_to_da_nang';
}
...
async function getDriverForBooking(passengerCount: number, driverId?: string | null) {
  if (driverId) {
    ...
    if (driver.availableSeats < passengerCount) {
      throw new Error('Selected vehicle does not have enough seats');
    }
...
export async function listBookings(userContext?: ActorContext) {
  ...
  const bookings = bookingStore.slice().sort((a, b) => b.createdAt.localeCompare(a.createdAt)).map(normalizeBooking);
  if (!userContext) {
    return bookings;
  }
...
export async function createRideBooking(input: {
  ...
  const booking: BookingRecord = normalizeBooking({
    id,
    customerName: customer?.fullName ?? 'Guest passenger',
