...
export function AuthForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="glass space-y-5 rounded-[1.75rem] p-5">
      ...
      {mode === 'register' ? (
        <div className="flex flex-wrap gap-2">
          {(['customer', 'driver'] as AuthRole[]).map((item) => (
            <button
              key={item}
              type="button"
          ...
          {[
            ...
            <label key={key} className="space-y-2 text-sm sm:col-span-1">
              ...
              <input
                ...
                onChange={(event) => setCustomerForm((prev) => ({ ...prev, [key]: event.target.value }))}
                className="w-full rounded-2xl border border-white/10 bg-slate-950/50 px-4 py-3 text-white outline-none focus:border-emerald-400/40"
              />
            </label>
          ))}
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            ...
            <label key={key} className="space-y-2 text-sm sm:col-span-1">
              <span className="text-slate-200">{label}</span>
              <input
                required={key !== 'vehiclePhoto' && key !== 'driverPhoto' && key !== 'description'}
                type={key === 'password' ? 'password' : 'text'}
                value={driverForm[key as keyof typeof driverForm]}
                onChange={(event) => setDriverForm((prev) => ({ ...prev, [key]: event.target.value }))}
                className="w-full rounded-2xl border border-white/10 bg-slate-950/50 px-4 py-3 text-white outline-none focus:border-emerald-400/40"
              />
            </label>
          ))}
