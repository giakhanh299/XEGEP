...
import { formatBookingStatus, formatTripStatus } from '@/lib/display-labels';
...
export function StatusBadge({ status }: { status: BookingStatus | TripStatus }) {
  ...
  const label = formatBookingStatus(status) === status ? formatTripStatus(status) : formatBookingStatus(status);
